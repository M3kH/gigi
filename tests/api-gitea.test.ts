...
const GITEA_ORG = process.env.GITEA_ORG || 'idea'
...
describe('lib/api-gitea', () => {
  ...
  describe('repos', () => {
    ...
    it('should get a specific repo', async () => {
      if (!GITEA_TOKEN) return
      const repo = await gitea.repos.get(GITEA_ORG, 'gigi')
    ...
    it('should list branches', async () => {
      if (!GITEA_TOKEN) return
      const branches = await gitea.repos.listBranches(GITEA_ORG, 'gigi')
    ...
    it('should list labels', async () => {
      if (!GITEA_TOKEN) return
      const labels = await gitea.repos.listLabels(GITEA_ORG, 'gigi')
  ...
  describe('issues', () => {
    ...
    it('should list issues', async () => {
      if (!GITEA_TOKEN) return
      const issues = await gitea.issues.list(GITEA_ORG, 'gigi', { limit: 5 })
    ...
    it('should get a specific issue', async () => {
      if (!GITEA_TOKEN) return
      const issue = await gitea.issues.get(GITEA_ORG, 'gigi', 56)
    ...
    it('should create an issue and comment on it', async () => {
      if (!GITEA_TOKEN) return
      const issue = await gitea.issues.create(GITEA_ORG, 'gigi', {
      ...
      // Comment on it
      const comment = await gitea.issues.createComment(GITEA_ORG, 'gigi', issue.number, 'Test comment from api-gitea')
  ...
  describe('pulls', () => {
    it('should list pull requests', async () => {
      if (!GITEA_TOKEN) return
      const prs = await gitea.pulls.list(GITEA_ORG, 'gigi', { limit: 5 })
  ...
  describe('orgs', () => {
    ...
    it('should get org by name', async () => {
      if (!GITEA_TOKEN) return
      const org = await gitea.orgs.get(GITEA_ORG)
      assert.equal(org.name, GITEA_ORG)
    })

    it('should list org repos', async () => {
      if (!GITEA_TOKEN) return
      const repos = await gitea.orgs.listRepos(GITEA_ORG)
      assert.ok(Array.isArray(repos), 'should return array')
  ...
  describe('errors', () => {
    it('should throw GiteaApiError on 404', async () => {
      if (!GITEA_TOKEN) return
      try {
        await gitea.repos.get(GITEA_ORG, 'nonexistent-repo-99999')
