...
describe('lib/api-gitea', () => {
  ...
  describe('repos', () => {
    ...
    it('should get a specific repo', async () => {
      if (!GITEA_TOKEN) return
      const repo = await gitea.repos.get('gigi', 'gigi')
    ...
    it('should list branches', async () => {
      if (!GITEA_TOKEN) return
      const branches = await gitea.repos.listBranches('gigi', 'gigi')
    ...
    it('should list labels', async () => {
      if (!GITEA_TOKEN) return
      const labels = await gitea.repos.listLabels('gigi', 'gigi')
  ...
  describe('issues', () => {
    ...
    it('should list issues', async () => {
      if (!GITEA_TOKEN) return
      const issues = await gitea.issues.list('gigi', 'gigi', { limit: 5 })
    ...
    it('should create an issue and comment on it', async () => {
      if (!GITEA_TOKEN) return
      const issue = await gitea.issues.create('gigi', 'gigi', {
      ...
      // Comment on it
      const comment = await gitea.issues.createComment('gigi', 'gigi', issue.number, 'Test comment from api-gitea')
    ...
    after(async () => {
      ...
      if (testIssueNumber && GITEA_TOKEN) {
        await gitea.issues.edit('gigi', 'gigi', testIssueNumber, { state: 'closed' })
      }
    })
  })
  ...
  describe('pulls', () => {
    it('should list pull requests', async () => {
      if (!GITEA_TOKEN) return
      const prs = await gitea.pulls.list('gigi', 'gigi', { limit: 5 })
  ...
  describe('orgs', () => {
    ...
    it('should get org by name', async () => {
      if (!GITEA_TOKEN) return
      const org = await gitea.orgs.get('gigi')
      assert.equal(org.name, 'gigi')
    })

    it('should list org repos', async () => {
      if (!GITEA_TOKEN) return
      const repos = await gitea.orgs.listRepos('gigi')
      assert.ok(Array.isArray(repos), 'should return array')
  ...
  describe('errors', () => {
    it('should throw GiteaApiError on 404', async () => {
      if (!GITEA_TOKEN) return
      try {
        await gitea.repos.get('gigi', 'nonexistent-repo-99999')
